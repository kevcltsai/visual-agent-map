...
5. Download the `main` branch versions of these four files into the plugin directory, replacing older copies only after the downloads succeed:
...
10. Verify the four installed plugin files, confirm that any existing `data.json` and `Agent Workspace/` were preserved, and report every command run plus the final paths. If a step fails, stop and report the error instead of claiming success.
